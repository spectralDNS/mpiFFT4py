...
curl_pad = empty((3,) + FFT.real_shape_padded())
U_pad   = empty((3,) + FFT.real_shape_padded())
...
def Cross(a, b, c):
    c[0] = FFT.fftn(a[1]*b[2]-a[2]*b[1], c[0], padded=True)
    c[1] = FFT.fftn(a[2]*b[0]-a[0]*b[2], c[1], padded=True)
    c[2] = FFT.fftn(a[0]*b[1]-a[1]*b[0], c[2], padded=True)
    return c

def Curl(a, c):
    c[2] = FFT.ifftn(1j*(K[0]*a[1]-K[1]*a[0]), c[2], padded=True)
    c[1] = FFT.ifftn(1j*(K[2]*a[0]-K[0]*a[2]), c[1], padded=True)
    c[0] = FFT.ifftn(1j*(K[1]*a[2]-K[2]*a[1]), c[0], padded=True)
    return c

def computeRHS(dU, rk):
    for i in range(3):
        U_pad[i] = FFT.ifftn(U_hat[i], U_pad[i], padded=True)
    curl_pad[:] = Curl(U_hat, curl_pad)
    dU = Cross(U_pad, curl_pad, dU)
