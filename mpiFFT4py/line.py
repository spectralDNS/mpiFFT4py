    ...
    def fft2(self, u, fu, dealias=None):
        ...
        if self.num_processes == 1:
            if not dealias == '3/2-rule':
                ...
                fu_padded[:] = rfft2(u/self.padsize**2, axes=(0,1),threads=self.threads)                
        ...
        if not dealias == '3/2-rule':
            ...
            # Transform in y-direction
            fu_padded_xy[:] = rfft(u/self.padsize, axis=1,threads=self.threads)
            ...
            U_sendr[:] = fft(U_sendr/self.padsize, axis=0,threads=self.threads)
    ...
    def ifft2(self, fu, u, dealias=None):
        ...
        if self.num_processes == 1:
            if not dealias == '3/2-rule':                
                ...
                u[:] = irfft2(fu_padded*self.padsize**2, axes=(0,1),threads=self.threads)
        ...
        if not dealias == '3/2-rule':
            ...
            fu_padded_x[:] = ifft(fu_padded_x, axis=0,threads=self.threads)
            ...
            u[:] = irfft(fu_padded_xy, axis=1,threads=self.threads)*self.padsize**2
